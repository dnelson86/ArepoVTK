...
Mutex::Mutex()
{
  if (pthread_mutex_init(&mutex, NULL))
    terminate("ERROR: pthread_mutex_init.");
}

Mutex::~Mutex()
{
  if (pthread_mutex_destroy(&mutex))
    terminate("ERROR: pthread_mutex_destroy.");
}
...
MutexLock::MutexLock(Mutex &m) : mutex(m)
{
  if (pthread_mutex_lock(&m.mutex))
    terminate("ERROR: pthread_mutex_lock.");
}

MutexLock::~MutexLock()
{
  if (pthread_mutex_unlock(&mutex.mutex))
    terminate("ERROR: pthread_mutex_unlock.");
}
...
{
  if (sem_close(sem))
    terminate("ERROR: sem_close.");
}

void Semaphore::Wait()
{
  if (sem_wait(sem))
    terminate("ERROR: sem_wait.");
}
...
void ConditionVariable::Signal()
{
  ...
  if ((err = pthread_cond_signal(&cond)) != 0)
    terminate("ERROR: cv_signal: %d", err);
}

void ConditionVariable::Unlock() {
  int err;
  if ((err = pthread_mutex_unlock(&mutex)) != 0)
    terminate("ERROR: cv_unlock: %d", err);
}
...
void TasksInit()
{
  ...
  for (int i = 0; i < nThreads; ++i) {
    ...
    if (err != 0)
      terminate("Renderer::TasksInit(): ERROR from pthread_create [%d]", err);
  }
...
void TasksCleanup()
{
  ...
  { MutexLock lock(*taskQueueMutex);
  if(taskQueue.size() != 0)
    terminate("TasksCleanup but have remaining taskQueue.size > 0");
  }
  ...
  if (threads != NULL) {
    for (int i = 0; i < nThreads; ++i) {
      ...
      if (err != 0)
        terminate("ERROR: tasks_cleanup pthread_join: %d", err);
    }
    delete[] threads;
    threads = NULL;
  }
